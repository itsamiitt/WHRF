...
const CMSEditor = {
    ...
    return `
      ...
      <div class="cms-layout">
        ...
        <div class="cms-preview-panel">
          ...
          <iframe id="cms-preview-iframe" class="cms-preview-iframe" src="../index.html" onload="CMSEditor.syncPreview()"
  ...
  schedulePreviewSync(forceReload = false) {
    clearTimeout(this.previewSyncTimer);
    this.previewSyncTimer = setTimeout(() => this.syncPreview(forceReload), forceReload ? 0 : 120);
  },

  syncPreview(forceReload = false) {
    const iframe = document.getElementById('cms-preview-iframe');
    if (!iframe || !this.config) return;
    ...
    try {
      iframe.contentWindow?.postMessage({
        type: 'wrhw-site-config-update',
        config: this.config
      }, '*');
    } catch {
      // Fallback to a hard refresh when direct messaging is not available yet.
      iframe.src = iframe.src;
    }
  },
  ...
  publishChanges() {
    ...
    UI.toast('Changes saved and preview refreshed.', 'success');
  },
